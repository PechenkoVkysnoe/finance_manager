...
#include "ui_calculatorgave.h"
#include "mainwindow.h"
#include "dialog3.h"

double numFirst1, numSecond1;

CalculatorGave::CalculatorGave(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::CalculatorGave)
{
    ui->setupUi(this);
...
CalculatorGave::~CalculatorGave()
{
    delete ui;
}

void CalculatorGave::onIdGave(int id)
{
    emit idSignal(id);
}

void CalculatorGave::digitsNum()
{
...
void CalculatorGave::on_pushButton_t_clicked()
{
...
void CalculatorGave::operation()
{
    QPushButton *button=(QPushButton *)sender();
    numFirst1=ui->label->text().toDouble();
    ui->label->setText("");
    button->setChecked(true);
}

void CalculatorGave::on_pushButton_equal_clicked()
{
    double labelNum;
    QString strLable;
    numSecond1 = ui->label->text().toDouble();
    if(ui->pushButton_plus->isChecked())
    {
        labelNum=numFirst1+numSecond1;
...
void CalculatorGave::on_pushButton_AC_clicked()
{
    ui->pushButton_mult->setChecked(false);
    ui->pushButton_plus->setChecked(false);
    ui->pushButton_minus->setChecked(false);
...
void CalculatorGave::on_pushButton_clicked()
{
    this->close();
    Dialog3 window;
    connect(this, SIGNAL(resultGot(double)), &window, SLOT(onResultGot(double)));
    connect(&window, SIGNAL(id(int)), this, SLOT(onIdGave(int)));
    emit resultGave(ui->label->text().toDouble());
